...
export async function POST(req: Request) {
  ...
  const { tasks, sections, storeIds, ...templateData } = body
  ...
  // CHK-1. Tasks used to be created nested inside this one `create`, which
  // cannot work now: a task needs a `sectionId`, and a section needs a
  // templateId that does not exist until the template row does. So the create
  // becomes a transaction — template, then sections, then tasks — rather than
  // one statement. Everything else about the shape is unchanged.
  //
  // `sections` is OPTIONAL on this route and normally absent: the Duplicate
  // button (templates-client.tsx) sends tasks only, and so would any older
  // client. `sectionsFromTasks` then recovers the order from MIN(orderIndex),
  // which is the migration's rule, so a duplicate carries its source's section
  // order without the client having to know sections exist.
  type IncomingTask = {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      const synced = await syncTemplateSections(tx, template.id, incomingSections)
      ...
      for (let i = 0; i < taskList.length; i++) {
        ...
        await tx.task.create({
          data: {
            ...
            // CHK-1: BOTH columns, always. `sectionId` is the entity;
            // `sectionName` is the legacy mirror kept written per TPL-1a's
            // shape (prisma/schema.prisma, Task.sectionName). Nothing reads the
            // string while the id is set.
            sectionName,
            sectionId: synced.byName.get(sectionName) ?? null,
      ...
      return tx.template.findUniqueOrThrow({
        ...
        // CHK-1: `orderBy` is NEW here and it is load-bearing, not tidiness.
        // The template form maps its pending attachments onto
        // `response.tasks[idx]` by position (template-form.tsx handleSave),
        // which held only because the nested `create` this replaces happened to
        // return insertion order. Tasks are now created in a loop, so the
